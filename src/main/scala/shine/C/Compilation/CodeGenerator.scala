...
import rise.core.substitute.{
  natInType => substituteNatInType,
  typeInType => substituteTypeInType
}
...
class CodeGenerator(
  ...
  override def translationContext: Compilation.TranslationContext =
  ...
  override def generate(
      ...
      env: Environment
  ): Phrase[CommType] => (immutable.Seq[Decl], Stmt) = phrase => {
      ...
      phrase match {
        ...
        case Assign(_, a, e) =>
          e |> exp(
            env,
            Nil,
            e => a |> acc(env, Nil, a => C.AST.ExprStmt(C.AST.Assignment(a, e)))
          )
        ...
        case LetNat(binder, defn, body) =>
          generateLetNat(binder, defn, env, (gen, env) => body |> gen.cmd(env))

        case Comment(comment) => C.AST.Comment(comment)

        // on the fly beta-reduction
        case Apply(fun, arg) =>
          Lifting.liftFunction(fun).reducing(arg) |> cmd(env)
        case DepApply(kind, fun, arg) =>
          arg match {
            case a: Nat =>
              Lifting.liftDependentFunction(
                fun.asInstanceOf[Phrase[`(nat)->:`[CommType]]]
              )(a) |> cmd(env)
            case a: DataType =>
              Lifting.liftDependentFunction(
                fun.asInstanceOf[Phrase[`(nat)->:`[CommType]]]
        ...
        case DMatchI(x, inT, _, f, dPair) =>
          dPair |> exp(
            ...
            input => {
              ...
              val sndId =
                Identifier[ExpType](freshName("sndId"), ExpType(sndT, `read`))
              val (sndCType, initT) = typ(sndT) match {
                case array: C.AST.ArrayType =>
                  (C.AST.PointerType(getBaseType(array)), (x: C.AST.Expr) => x)
                case t =>
                  (
                    t,
                    (x: C.AST.Expr) => C.AST.UnaryExpr(C.AST.UnaryOperator.*, x)
                  )
              }
              ...
              C.AST.Block(
                immutable.Seq(
                    ...
                    C.AST.VarDecl(
                      ...
                      Some(
                        C.AST.ArraySubscript(
                          C.AST.Cast(C.AST.PointerType(C.AST.Type.u32), input),
                          C.AST.Literal("0")
                        )
                      )
                    )
                  ...
                  f(fstId)(sndId) |> cmd(
                    env.updatedIdentEnv((sndId, C.AST.DeclRef(sndId.name)))
                  )
                )
              )
            }
              ...
              a |> acc(
                ...
                expr =>
                  C.AST.ExprStmt(
                    C.AST.Assignment(
                      C.AST.ArraySubscript(
                        C.AST.Cast(C.AST.PointerType(C.AST.Type.u32), expr),
                        C.AST.Literal("0")
                      ),
                      fst
                    )
        ...
        case Apply(_, _) | DepApply(_, _, _) | _: CommandPrimitive =>
  ...
  override def acc(
    ...
    case SplitAcc(n, _, _, a) =>
      path match {
        case (i: CIntExpr) :: ps =>
          a |> acc(env, CIntExpr(i / n) :: CIntExpr(i % n) :: ps, cont)
      ...
      path match {
        ...
        case Nil             =>
          ...
          fst |> acc(
            env,
            Nil,
            { case C.AST.StructMemberAccess(a1, _) =>
              snd |> acc(
                env,
                Nil,
                { case C.AST.StructMemberAccess(a2, _) =>
                  assert(a1 == a2)
                  cont(a1)
                }
              )
    ...
    case UnzipAcc(_, _, _, a) =>
      path match {
        case (i: CIntExpr) :: FstMember :: ps =>
          a |> acc(env, FstMember :: i :: ps, cont)
        case (i: CIntExpr) :: SndMember :: ps =>
          a |> acc(env, SndMember :: i :: ps, cont)
    ...
    case MapAcc(n, dt, _, f, a) =>
      path match {
        case (i: CIntExpr) :: ps =>
          f(IdxAcc(n, dt, functional.NatAsIndex(n, Natural(i)), a)) |> acc(
            env,
            ps,
            cont
          )
  ...
  override def exp(
    ...
    case uop @ UnaryOp(op, e) =>
      uop.t.dataType match {
        case _: ScalarType =>
          path match {
            case Nil =>
              e |> exp(env, Nil, e => cont(CCodeGen.codeGenUnaryOp(op, e)))
    ...
    case Split(n, _, _, _, e) =>
      path match {
        case (i: CIntExpr) :: (j: CIntExpr) :: ps =>
          e |> exp(env, CIntExpr(n * i + j) :: ps, cont)
        case _ => error(s"Expected two C-Integer-Expressions on the path.")
      }
    case Join(_, m, _, _, e) =>
      path match {
      ...
      path match {
        case (i: CIntExpr) :: ps =>
          functional.Idx(
            ...
            dt,
            functional
              .Idx(m, IndexType(n), functional.NatAsIndex(m, Natural(i)), y),
            e
          ) |>
            exp(env, ps, cont)
        case _ => error(s"unexpected $path")
    ...
    case Slide(_, _, s2, _, e) =>
      path match {
        case (i: CIntExpr) :: (j: CIntExpr) :: ps =>
          e |> exp(env, CIntExpr(i * s2 + j) :: ps, cont)
        case _ => error(s"Expected two C-Integer-Expressions on the path.")
      }

    case Transpose(_, _, _, _, e) =>
      path match {
        case (i: CIntExpr) :: (j: CIntExpr) :: ps =>
          e |> exp(env, j :: i :: ps, cont)
        case _ => error(s"did not expect $path")
      }
    ...
    case TransposeDepArray(_, _, _, e) =>
      path match {
        case (i: CIntExpr) :: (j: CIntExpr) :: ps =>
          e |> exp(env, CIntExpr(j) :: CIntExpr(i) :: ps, cont)
        case _ => error(s"Expected two C-Integer-Expressions on the path.")
      }

    case Map(n, dt, _, _, f, e) =>
      path match {
        case (i: CIntExpr) :: ps =>
          f(Idx(n, dt, functional.NatAsIndex(n, Natural(i)), e)) |> exp(
            env,
            ps,
            cont
          )
      ...
      path match {
        case (i: CIntExpr) :: ps =>
          ...
          f(
            ...
            env updatedContEnv (continue_cmd -> (e =>
              env => e |> exp(env, ps, cont)
            ))
          )
        case _ => error(s"Expected path to be not empty")
      }
    ...
    case ffc @ ForeignFunctionCall(f, _) =>
      CCodeGen.codeGenForeignFunctionCall(
        f,
        ffc.inTs,
        ffc.outT,
        ffc.args,
        env,
        fe => generateAccess(ffc.outT, fe, path, env, cont)
      )

    case Proj1(pair) =>
      SimplifyNats.simplifyIndexAndNatExp(Lifting.liftPair(pair)._1) |> exp(
        env,
        path,
        cont
      )
    case Proj2(pair) =>
      SimplifyNats.simplifyIndexAndNatExp(Lifting.liftPair(pair)._2) |> exp(
        env,
        path,
        cont
      )
  ...
  override def typ(dt: DataType): Type = {
    def typeToStructNameComponent(t: DataType): String = {
      t match {
        ...
        case _: ScalarType | _: FragmentType | _: IndexType | `NatType` |
            _: VectorType =>
          typ(t).toString
        case _ =>
          throw new Exception(
            s"Can't convert data type $t to struct name component"
          )
      }
    }

    dt match {
      case b: rise.core.types.DataType.ScalarType =>
        b match {
        ...
        a.fdt match {
          case NatToDataLambda(_, body) =>
            C.AST.ArrayType(
              typ(body),
              Some(a.size)
            ) // TODO: be more precise with the size?
          case _: NatToDataIdentifier =>
            throw new Exception("This should not happen")
        }
      case r: rise.core.types.DataType.PairType =>
        C.AST.StructType(
          "Record_" + typeToStructNameComponent(
            r.dt1
          ) + "_" + typeToStructNameComponent(r.dt2),
          immutable.Seq((typ(r.dt1), "_fst"), (typ(r.dt2), "_snd"))
        )
      case rise.core.types.DataType.DepPairType(_, _, _) =>
        C.AST.PointerType(C.AST.Type.u8)
      case _: rise.core.types.DataType.DataTypeIdentifier |
          _: rise.core.types.DataType.NatToDataApply |
          rise.core.types.DataType.ManagedBufferType(_) |
          rise.core.types.DataType.OpaqueType(_) =>
        ...
        dt match {
          case DepPairType(_, _, sndT) =>
            generateAccess(
              sndT,
              C.AST.Cast(
                C.AST.PointerType(C.AST.Type.getBaseType(typ(sndT))),
                C.AST.BinaryExpr(
                  expr,
                  C.AST.BinaryOperator.+,
                  C.AST.Literal("sizeof(uint32_t)")
                )
              ),
              ps,
              env,
          ...
          case other =>
            throw new Exception(
              s"Expected a Dependent Pair but $other found instead"
            )
        }
      case _ =>
        throw new Exception(
  ...
  def generateInlinedCall[T <: PhraseType](
    ...
    def error(s: String) = throw new Exception(
      s + " in deferred function generation"
    )
    phrase match {
      ...
      case ndl: DepLambda[Nat, NatIdentifier, _] @unchecked =>
        args.headOption match {
          case Some(Right(nat)) =>
  ...
  /** This function takes a phrase with a nat dependent free variable `for`, and
    * it generates a block where `for` is bound to the arithmetic expression at.
    * @param `for`
    *   The free nat variable to substitute
    * @param phrase
    *   The phrase to generate
    * @param at
    *   The arithmetic expression we are generating phrase at
    * @param env
    ...
    shine.DPIA.Types.substitute(at, `for`, in = phrase) |> (p => {
      val newIdentEnv = env.identEnv.map {
        case (Identifier(name, AccType(dt)), declRef) =>
          (
            Identifier(name, AccType(substituteNatInType(at, `for`, in = dt))),
            declRef
          )
        case (Identifier(name, ExpType(dt, a)), declRef) =>
          (
            Identifier(
              name,
              ExpType(substituteNatInType(at, `for`, in = dt), a)
            ),
            declRef
          )
        case x => x
      ...
      C.AST.Block(
        immutable.Seq(p |> this.cmd(env.copy(identEnv = newIdentEnv)))
      )
    })
  }
  ...
  protected object CCodeGen {
    ...
    def codeGenIdx(
      ...
      i |> exp(
        ...
        {
          case C.AST.DeclRef(name) =>
            e |> exp(env, CIntExpr(NamedVar(name, ranges(name))) :: ps, cont)
          case C.AST.ArithmeticExpr(ae) =>
            e |> exp(env, CIntExpr(ae) :: ps, cont)
          case cExpr: C.AST.Expr =>
        ...
        case Some(funDef) =>
          addDeclaration(
            C.AST.FunDecl(
              funDecl.name,
              returnType = typ(outT),
              params = (funDef.params zip inTs).map { case (name, dt) =>
                C.AST.ParamDecl(name, typ(dt))
              },
      ...
      dt.fdt match {
        case NatToDataLambda(x, body) =>
          BigSum(
            ...
            upTo = at - 1,
            `for` = x,
            types.DataTypeOps.getTotalNumberOfElements(body)
          )
        case _: NatToDataIdentifier =>
          throw new Exception(s"This should not happen")
      }
  ...
  override def genNat(n: Nat, env: Environment, cont: Expr => Stmt): Stmt = {
    ...
    /** General form of expression generation algorithm from a series of nats,
      ...
      *
      * @param nats
      *   The series of nats from which the expression is generated
      * @param op
      *   The binary operator used
      * @param default
      *   In case the nat list is empty, we shall return this value
      * @param cont
      *   The cont of the generation
      * @param accum
      *   Internal accumulator used across iterations
      * @return
      */
    def genBinopFold(
        nats: Iterable[Nat],
        ...
        ex match {
          case Cst(-1) =>
            ...
            genNat(
              ...
              b =>
                cont(
                  C.AST.BinaryExpr(AST.Literal("1"), C.AST.BinaryOperator./, b)
                )
            )
          case Cst(2) =>
            genNat(
